...
import { DinerProfile, DinerVisit, DinerReview, DinerBadge } from "@/types/database";
...
export default function DinerProfileContent({ initialRestaurantId }: DinerProfileContentProps) {
  ...
  const [selectedVisitForReview, setSelectedVisitForReview] = useState<string | null>(null);
  ...
  const loadDinerData = useCallback(async () => {
    ...
    try {
      ...
      // Load profile data
      const [profileResponse, visitsResponse, reviewsResponse, badgesResponse] = await Promise.all([
                  ...
                  <div className="flex items-center gap-4">
                    <Avatar className="h-20 w-20">
                      ...
                      <AvatarFallback className="text-lg">
                        {profile?.display_name?.charAt(0) || user?.name?.charAt(0) || "U"}
                      </AvatarFallback>
                    </Avatar>
                    <div>
                      <CardTitle className="text-2xl">{profile?.display_name || user?.name}</CardTitle>
                      <CardDescription className="flex items-center gap-2 mt-1">
                        <Calendar className="h-4 w-4" />
                        Member since {profile ? formatJoinDate(profile.join_date) : "today"}
                      </CardDescription>
                      <div className="flex items-center gap-1 mt-2">
                        {profile?.is_public ? (
                          <><Eye className="h-4 w-4 text-green-600" /><span className="text-sm text-green-600">Public Profile</span></>
                        ) : (
                          <><EyeOff className="h-4 w-4 text-gray-500" /><span className="text-sm text-gray-500">Private Profile</span></>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              {editMode && (
                <CardContent>
                  <form onSubmit={handleUpdateProfile} className="space-y-4 max-w-md">
                    ...
                    <Button type="submit" disabled={saving} className="bg-[#5F7161] hover:bg-[#4C5B4F]">
                      {saving ? (
                        <>
                  ...
                  <CardContent>
                    {badges.length === 0 ? (
                      <div className="text-center py-8 text-gray-500">
                        ...
                        <p>Keep visiting restaurants and writing reviews to earn badges!</p>
                      </div>
                    ) : (
                        ...
                        {badges.map((badge) => (
                          <div key={badge.id} className="border rounded-lg p-4 text-center">
                            <div className="flex justify-center mb-3">
                              {badge.badge_type === 'champion' && <Crown className="h-8 w-8 text-yellow-500" />}
                              {badge.badge_type === 'reviewer' && <Star className="h-8 w-8 text-blue-500" />}
                              {badge.badge_type === 'photographer' && <Camera className="h-8 w-8 text-green-500" />}
                              {badge.badge_type === 'frequent_visitor' && <Heart className="h-8 w-8 text-red-500" />}
                              {badge.badge_type === 'explorer' && <MapPin className="h-8 w-8 text-purple-500" />}
                              {badge.badge_type === 'first_visit' && <Trophy className="h-8 w-8 text-orange-500" />}
                            </div>
