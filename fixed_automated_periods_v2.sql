-- Fixed Automated Leaderboard Period Management (Version 2)
-- This resolves the qualified SET column error

-- 1. Create a comprehensive period management function (FIXED)
...
BEGIN
    ...
    -- 1. Close any expired periods (FIXED: don't qualify SET columns)
    UPDATE leaderboard_periods 
    SET status = 'completed', updated_at = NOW()
    WHERE leaderboard_periods.status = 'active' AND leaderboard_periods.end_date < current_monday;
    ...
    IF closed_periods > 0 THEN
        ...
        WHERE lp.status = 'completed' AND lp.end_date < current_monday
    ...
    -- 5. Activate upcoming periods that should be active now (FIXED: don't qualify SET columns)
    UPDATE leaderboard_periods 
    SET status = 'active', updated_at = NOW()
    WHERE leaderboard_periods.status = 'upcoming' 
      AND leaderboard_periods.start_date <= CURRENT_DATE 
      AND leaderboard_periods.end_date >= CURRENT_DATE;
